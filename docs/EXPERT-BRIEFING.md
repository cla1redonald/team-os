...
- *"But it already exists somewhere a connector can reach — why recreate it as a file?"* → **The kernel isn't a copy of your content; it's the register of which content is canonical.** Usually "it exists" means six versions exist in six places — and the fact that would resolve them (*which one counts*) exists nowhere; the kernel file is that election, not a recreation. Where something genuinely exists once and well, **point, don't copy**: four lines — canonical source, owner, last affirmed, anything-disagreeing-is-stale (see `processes/enterprise-client-onboarding.md` for the pattern). Think **DNS for meaning**: DNS doesn't duplicate the website, it authoritatively resolves the name. And drift reverses: without the register the sources drift from each other with no arbiter; with it there's one small, owned, journaled drift surface — which agents, reading it on every task, keep honest. Kernel files are thin by design: elections, pointers, and tacit rules — not content.
- *"Who actually does this work — isn't this a librarian?"* → Yes, and we've run this experiment before: the 1990s knowledge-management wave hired corporate librarians, and it died because **curation had no reader**. The agent is the tireless reader that consults the register on every task — the librarian is back, and this time she has a customer. Shape matters: **librarian = registrar, not oracle**. Domain owners own the elections (finance owns what churn means); the librarian owns the register's health — structure, cross-links, staleness sweeps, chasing owners to affirm-or-amend. And most of that patrol is agent-automatable: human librarian for judgment, agent librarian for hygiene.
- *"We already share skills across the team — isn't that this?"* → A skill tells agents **how**; the kernel tells them **what's true**. Procedure register vs truth register — same substrate (portable markdown), different asset. Skill *distribution* is solved and buyable (cross-tool sync platforms exist — a driver, buy it). Skill *governance* is not: who owns a skill, when is one superseded, what patrols the library for rot? The register pattern — own, elect, supersede, patrol — applies to both. *You need both registers, and both need owners.*
